...
禁止使用 P0/P1/P2，统一采用以下四级体系：

- S 级（必须修改）：可能影响效力、导致重大财政/审计/追责风险，或不改无法落地执行。签署前优先处理，必须给替代条款。
- A 级（重点修改）：会削弱委托方控制权或显著增加争议、履约成本。给修改文本。
- B 级（建议优化）：表述不够严谨，但不构成重大风险。给优化建议。
- C 级（提醒关注）：需结合事实确认，合同文本本身未必错误。提出核实问题。
...
- **审查者风格档案（最高优先级）**：`references/reviewer-style-profile.md` — 覆盖通用逻辑，审查时优先对照此文件
